---
layout: post
title: 万字长文总结分布式事务，总有一款适合你(转)
tags:
- 分布式系统
categories: distribute-systems
description: 万字长文总结分布式事务，总有一款适合你
---
...
目前主要的解决方案有2种，一种是```本地消息表方案```，一种是```事务消息方案```。
...
果业务需要隔离，通常就应该重点考虑 TCC（Try-Confirm-Cancel）方案，TCC天生适用于需要强隔离性的分布式事务中，它是由数据库专家帕特 · 赫兰德（Pat Helland）在 2007 年撰写的论文《Life beyond Distributed Transactions: An Apostate’s Opinion》中提出的。
...
### 5.1 基于状态的补偿
这是一个根据业务特性进行妥协的一种方案，根据实际的业务场景对数据重要性进行划分，放弃传统的全局数据一致，允许部分不重要的数据出现不一致，但不会对业务产生重大影响。
...
### 7.2 DTM 
DTM是一款golang开发的分布式事务管理器(https://github.com/dtm-labs/dtm)，目前star数4.6k，它解决了跨数据库、跨服务、跨语言栈更新数据的一致性问题。他优雅的解决了幂等、空补偿、悬挂等分布式事务难题，提供了简单易用、高性能、易水平扩展的解决方案。
...
1. [万字长文总结分布式事务，总有一款适合你](https://zhuanlan.zhihu.com/p/461294722)
