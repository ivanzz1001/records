...
从上述查找过程可见，在查找成功时走了一条从根到叶子节点的路径。例如，在上图中查找```CHEN```的过程为： 从根节点```α```出发，经```β、γ```节点，最后到达叶子节点```δ```。而查找```CHAI```的过程为从根节点```α```出发，经```β、γ```节点后到达```ε```节点, 由于该节点中和字符```I```相应为空，则查找不成功。由此，其查找的时间依赖于树的深度。我们可以对关键字选择一种合适的分割，以缩减```Trie树```的深度。
