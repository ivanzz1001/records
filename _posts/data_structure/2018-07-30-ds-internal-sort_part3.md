...
* 上表中的```简单排序```包括除```希尔排序```之外的所有插入排序、冒泡排序和简单选择排序，其中以直接插入排序为最简单，当序列中的记录```基本有序```或n值较小时，它是最佳的排序方法，因此长将它和其他的排序方法，诸如快速排序、归并排序等结合在一起使用。
...
综上所述，在前面所讨论的 所有排序方法中，没有哪一种是绝对最优的。有的适用于```n```较大的情况，有的适用于```n```较小的情况，有的...，因此，在实用使需根据不同情况适当选用，甚至可将多种方法结合起来使用。
...
最后在需要时可根据adr的值重排记录的物理位置，重排算法如下：
{% highlight string %}
